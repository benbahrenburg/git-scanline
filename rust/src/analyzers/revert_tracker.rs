...
pub fn analyze_reverts(commits: &[Commit], files: &[String]) -> HashMap<String, RevertData> {
    ...
    for commit in commits {
        if !REVERT_PATTERN.is_match(commit.subject.trim()) { continue; }
    ...
    let max_reverts = file_reverts.values().cloned().fold(0.0001_f64, |a, b| a.max(b as f64));

    files.iter().map(|file| {
        let count = file_reverts.get(file).cloned().unwrap_or(0);
        (file.clone(), RevertData {
            revert_count: count,
...
mod tests {
    ...
    fn test_detects_revert_commits() {
        ...
        assert_eq!(result["src/feature.rs"].revert_count, 1, "feature.rs should have 1 revert");
        assert_eq!(result["src/other.rs"].revert_count, 0, "other.rs should have 0 reverts");
    }
    ...
    fn test_revert_detection_is_case_insensitive() {
        for prefix in &["Revert", "revert", "REVERT"] {
            ...
            let files = vec!["a.rs".to_string()];
            let result = analyze_reverts(&commits, &files);
            assert!(result["a.rs"].revert_count > 0,
                "prefix '{}' should be detected as a revert", prefix);
        }
    }
    ...
    fn test_revert_score_in_range() {
        ...
        for (_, data) in &result {
            assert!(data.revert_score >= 0.0 && data.revert_score <= 100.0,
                "revert_score {} out of range", data.revert_score);
        }
    }
}
