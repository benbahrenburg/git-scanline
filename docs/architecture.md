...
The Rust binary lives at the workspace root. It is optimized for speed with a single git subprocess and parallel analyzer execution via `rayon`.
...
├── filters.rs       File noise filter (globs, path prefixes)
├── git/
