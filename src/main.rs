...
struct Args {
    ...
    /// Run `git-scanline --generate-config` to print an annotated template.
    #[arg(long, value_name = "FILE")]
    config: Option<PathBuf>,

    /// Print a .git-scanline.yml config template to stdout and exit.
    /// Example: git-scanline --generate-config > .git-scanline.yml
    #[arg(long)]
    generate_config: bool,

    #[arg(long = "weight-churn", default_value_t = 0.27)]
    weight_churn: f64,
    #[arg(long = "weight-bugs", default_value_t = 0.27)]
    weight_bugs: f64,
    #[arg(long = "weight-reverts", default_value_t = 0.14)]
    weight_reverts: f64,
    #[arg(long = "weight-bursts", default_value_t = 0.09)]
    weight_bursts: f64,
    #[arg(long = "weight-coupling", default_value_t = 0.09)]
    weight_coupling: f64,
    #[arg(long = "weight-silo", default_value_t = 0.05)]
    weight_silo: f64,
    #[arg(long = "weight-commit-quality", default_value_t = 0.09)]
    weight_commit_quality: f64,
}
...
fn main() {
    ...
    if args.generate_config {
        if let Err(e) = config::print_template(None) {
            eprintln!("Error: {e}");
            std::process::exit(1);
        }
        return;
    ...
    let cfg = match &args.config {
        ...
        None => config::ScanlineConfig::default(),
    };

    // Merge config into args — config values fill in fields still at their defaults.
    // Explicit CLI flags take precedence because we only apply config when the
    // arg value equals its hard-coded default.
    if args.since.is_empty() {
        if let Some(v) = cfg.since.as_deref() {
            args.since = v.to_string();
        }
    }
    if args.path.is_none() {
        args.path = cfg.path.clone();
    }
    if args.top == 20 {
        if let Some(v) = cfg.top {
            args.top = v;
        }
    }
    if !args.bugs_only {
        if let Some(v) = cfg.bugs_only {
            args.bugs_only = v;
        }
    }
    if args.format == "terminal" {
        if let Some(v) = cfg.format.as_deref() {
            args.format = v.to_string();
        }
    }
    if args.output.is_none() {
        if let Some(v) = &cfg.output {
            args.output = Some(PathBuf::from(v));
        }
    }
    if let Some(w) = &cfg.weights {
        if args.weight_churn == 0.27 {
            if let Some(v) = w.churn {
                args.weight_churn = v;
            }
        }
        if args.weight_bugs == 0.27 {
            if let Some(v) = w.bugs {
                args.weight_bugs = v;
            }
        }
        if args.weight_reverts == 0.14 {
            if let Some(v) = w.reverts {
                args.weight_reverts = v;
            }
        }
        if args.weight_bursts == 0.09 {
            if let Some(v) = w.bursts {
                args.weight_bursts = v;
            }
        }
        if args.weight_coupling == 0.09 {
            if let Some(v) = w.coupling {
                args.weight_coupling = v;
            }
        }
        if args.weight_silo == 0.05 {
            if let Some(v) = w.silo {
                args.weight_silo = v;
            }
        }
        if args.weight_commit_quality == 0.09 {
            if let Some(v) = w.commit_quality {
                args.weight_commit_quality = v;
            }
        }
    }
    let filter_overrides = cfg.filter_overrides();
...
fn run_analysis(
    ...
    args: &Args,
    weights: &Weights,
    filter_overrides: &config::FilterOverrides,
    ...
    let (commits, diff_stats) =
        match git::log_parser::parse_log(repo_path, &args.since, args.path.as_deref()) {
    ...
    pb.println(format!(
        "  ✓ [1/5] Parsing commit log + diff stats       {t1}"
    ));
    ...
    step_start = Instant::now();
    pb.println(format!(
        "  ✓ [2/5] Scanning for security risks           {t2}"
    ));

    pb.set_message(format!("{}[3/5] Filtering files...", pfx));
    let all_files: HashSet<String> = commits
        .iter()
        ...
        || {
            rayon::join(
                ...
                || {
                    rayon::join(
                        || {
                            analyzers::bug_correlation::analyze_bug_correlation(
                                &commits,
                                &filtered_files,
                            )
                        },
                    ...
                    rayon::join(
                        ...
                        || {
                            rayon::join(
                                ...
                                || {
                                    analyzers::commit_quality::analyze_commit_quality(
                                        &commits,
                                        &filtered_files,
                                    )
                                },
    ...
    step_start = Instant::now();
    pb.println(format!(
        "  ✓ [4/5] All 7 analyzers (parallel)            {t4}"
    ));
    ...
    if args.bugs_only {
        results.retain(|r| r.details.bug_commits > 0);
    }
    ...
    let top_couplings: Vec<CouplingEntry> = coupling_data
        .into_iter()
    ...
    match args.format.as_str() {
        ...
        _ => {
            if is_multi {
                ...
                println!();
                println!("╔══════════════════════════════════════════════════════╗");
                let path_preview: String = format!("({})", repo_path.display())
                    .chars()
                    .take(45)
                    .collect();
                println!("║  📁 {}  {}", repo_name, path_preview);
                println!("╚══════════════════════════════════════════════════════╝");
            }
    ...
    if interactive_mode && args.format == "terminal" {
        let export_answer = prompt("  Output this report to file? [no]: ");
        if matches!(export_answer.trim().to_lowercase().as_str(), "yes" | "y") {
            let default_output = if is_multi {
                format!("hotspot-report-{repo_name}.html")
            } else {
                "hotspot-report.html".to_string()
            };
            let output_input = prompt(&format!("  Output path [{}]: ", default_output));
            let mut export_path = if output_input.trim().is_empty() {
                PathBuf::from(default_output)
            } else {
                PathBuf::from(output_input.trim().trim_matches('"').trim_matches('\''))
            };
            ...
            if ext == "json" {
                reporters::json::report_json(&report, Some(&export_path))?;
            } else {
                if ext.is_empty() {
                    export_path.set_extension("html");
                } else if ext != "html" && ext != "htm" {
                    println!("  ⚠  Unknown extension '.{}' — exporting as HTML", ext);
                }
                reporters::html::report_html(&report, &export_path)?;
            }
        }
    }
...
fn run_interactive(mut args: Args) -> Args {
    ...
    let since_display = if args.since.is_empty() {
        "all history".to_string()
    } else {
        args.since.clone()
    };
    ...
    // Interactive mode defaults to terminal output, then offers export after report generation.
    args.format = "terminal".to_string();
    args.output = None;

    let input = prompt(&format!(
        "  Top N results to show in report (all files are always scanned) [{}]: ",
        args.top
    ));
    if !input.trim().is_empty() {
        args.top = input.trim().parse().unwrap_or(args.top);
    }

    let input = prompt(&format!(
        "  Bugs-only mode [{}] (yes/no): ",
        if args.bugs_only { "yes" } else { "no" }
    ));
    if !input.trim().is_empty() {
    ...
    if !input.trim().is_empty() {
        args.path = Some(input.trim().to_string());
    }
    ...
    if matches!(customize.trim().to_lowercase().as_str(), "yes" | "y") {
        ...
        args.weight_churn = prompt_f64(
            &format!("    churn          [{:.2}]: ", args.weight_churn),
            args.weight_churn,
        );
        args.weight_bugs = prompt_f64(
            &format!("    bugs           [{:.2}]: ", args.weight_bugs),
            args.weight_bugs,
        );
        args.weight_reverts = prompt_f64(
            &format!("    reverts        [{:.2}]: ", args.weight_reverts),
            args.weight_reverts,
        );
        args.weight_bursts = prompt_f64(
            &format!("    bursts         [{:.2}]: ", args.weight_bursts),
            args.weight_bursts,
        );
        args.weight_coupling = prompt_f64(
            &format!("    coupling       [{:.2}]: ", args.weight_coupling),
            args.weight_coupling,
        );
        args.weight_silo = prompt_f64(
            &format!("    silo           [{:.2}]: ", args.weight_silo),
            args.weight_silo,
        );
        args.weight_commit_quality = prompt_f64(
            &format!("    commit-quality [{:.2}]: ", args.weight_commit_quality),
            args.weight_commit_quality,
        );
    }
...
mod tests {
    ...
    /// Loads the workspace `.env` file and returns a map of key → value.
    fn load_env() -> std::collections::HashMap<String, String> {
        let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
        let env_path = if manifest_dir.join(".env").exists() {
            manifest_dir.join(".env")
        } else {
            manifest_dir.parent().unwrap_or(&manifest_dir).join(".env")
        };
    ...
    fn test_fmt_dur_milliseconds() {
        ...
        assert!(
            s.ends_with("ms"),
            "Sub-second durations should use 'ms': got '{s}'"
        );
        assert!(
            s.contains("250"),
            "Should show the millisecond value: got '{s}'"
        );
    }
    ...
    fn test_fmt_dur_seconds() {
        ...
        assert!(
            s.ends_with('s'),
            "Durations >= 1s should use 's': got '{s}'"
        );
    ...
    fn test_parse_log_real_repo() {
        ...
        assert!(
            !commits[0].author.is_empty(),
            "Commit should have an author"
        );
    }
    ...
    fn test_full_pipeline_scores_in_range() {
        ...
        let (commits, _) =
            git::log_parser::parse_log(&repo, "", None).expect("parse_log should succeed");
        ...
        let all_files: HashSet<String> = commits
            ...
            .flat_map(|c| c.files.iter().cloned())
            .collect();
        let files = filters::filter_files(
            &all_files.into_iter().collect::<Vec<_>>(),
            None,
            &config::FilterOverrides::default(),
        );
        ...
        for r in &results {
            assert!(
                r.hotspot_score >= 0.0 && r.hotspot_score <= 100.0,
    ...
    fn test_security_detects_env_files() {
        ...
        assert!(
            flagged.contains(&".env.production"),
            ".env.production must be flagged"
        );
        assert!(
            !flagged.contains(&"src/app.rs"),
            "src/app.rs must not be flagged"
        );
    }
}
