...
use crate::types::{Commit, BurstData};
...
pub fn analyze_bursts(commits: &[Commit], files: &[String]) -> HashMap<String, BurstData> {
    ...
    for commit in commits {
        for file in &commit.files {
            ...
            file_timestamps.entry(file.clone()).or_default().push(commit.timestamp);
        }
    }
    ...
    let max_bursts = raw.values().cloned().fold(0.0001_f64, |a, b| a.max(b as f64));

    files.iter().map(|file| {
        let incidents = *raw.get(file).unwrap_or(&0);
        (file.clone(), BurstData {
...
mod tests {
    ...
    fn test_detects_burst_within_24h() {
        ...
        assert!(result["hot.rs"].burst_incidents > 0, "hot.rs should have a burst incident");
        assert_eq!(result["cold.rs"].burst_incidents, 0, "cold.rs has only one commit, no burst");
    }
    ...
    fn test_no_burst_for_sparse_commits() {
        ...
        let result = analyze_bursts(&commits, &files);
        assert_eq!(result["a.rs"].burst_incidents, 0, "Sparse commits should not register as a burst");
    }
    ...
    fn test_burst_requires_at_least_3_commits() {
        ...
        let files = vec!["a.rs".to_string()];
        let result = analyze_bursts(&commits, &files);
        assert_eq!(result["a.rs"].burst_incidents, 0, "Two commits within 24h should not count as a burst");
    }
    ...
    fn test_burst_score_in_range() {
        ...
        assert!(score >= 0.0 && score <= 100.0, "burst_score {} out of range", score);
    }
}
